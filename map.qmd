...
# Create Civic Opportunity Map
map1 <- leaflet(map_data_civic) %>%
  ...
  addLegend(pal = pal_green, values = ~civic_opp_category, title = "Civic Opportunity Index") %>%
  fitBounds(lng1 = -125, lat1 = 24, lng2 = -66.9, lat2 = 49)  # Continental US bounding box


# Create Organization Type Map
map2 <- leaflet(map_data_orgtype) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
  ...
  addLegend(pal = pal_primary_org, values = ~primary_org_class, title = "Primary Organization Type") %>%
  fitBounds(lng1 = -125, lat1 = 24, lng2 = -66.9, lat2 = 49)  # Continental US bounding box
```

```{r, echo=FALSE}
# Create a combined Leaflet map with layer control
combined_map <- leaflet(options = leafletOptions(minZoom = 3, zoomControl = TRUE)) %>%
  ...
  # Add Primary Civic Opportunity Organization Type map layer
  addPolygons(
    ...
    label = ~NAMELSAD,
    group = "Primary Civic Opportunity Organization Type"
  ) %>%
  
  # Add layer control for toggling between maps
  addLayersControl(
    baseGroups = c("Civic Opportunity Counts", "Primary Civic Opportunity Organization Type"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  
  # Set initial view to the continental US
