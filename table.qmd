...
df_joined <- df %>%
  ...
  select(
    State = state_abbr,
    County = county_name,
    `Civic Opportunity per Capita` = civic_opp_sum_normalized,
...
DT::datatable(
  ...
  options = list(
    ...
    columnDefs = list(
      ...
      list(className = 'highlight-civic', targets = 2:12),
      list(className = 'highlight-type', targets = 13:(ncol(df_joined) - 1))
    )
  ),
  filter = "top",
